...
def create_plot_parser(generate_help=True):
    parser = argparse.ArgumentParser(description='Analyse the given CSV file.', parents=[__create_common_parser()], add_help=generate_help)

    parser.add_argument('columns', type=str, nargs='+',
                         help='A list of columns that should be plotted. Either the column name or index are valid.', default=[])
    parser.add_argument('-d', '--divide',       type=__divide_check,
...
def create_utility_parser(generate_help=True):
    parser = argparse.ArgumentParser(description='Provide utilities to process a CSV file in different ways.', parents=[__create_common_parser()], add_help=generate_help)

    parser.add_argument('-l', '--list-headers', action='store_true', help='List all column headers found (all entries of the first row).', default=False)
    return parser
